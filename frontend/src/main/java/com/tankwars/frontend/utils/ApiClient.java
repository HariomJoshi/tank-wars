...
public class ApiClient {
    ...
    public CompletableFuture<List<String>> sendGetReq(String query) {
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(query)) // Use the full URL with query parameters
                .timeout(Duration.ofSeconds(10))
                .header("Content-Type", "application/json") // Set the correct content type if needed
        ...
        return client.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                .thenApply(response -> {
                    System.out.println("Response: " + response.body());
                    if (response.statusCode() == 200) { // Return true if status code is 200 (OK)
                        List<String> data = gson.fromJson(response.body(), List.class);
